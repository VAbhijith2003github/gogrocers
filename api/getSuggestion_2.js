...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const query = `
    You are a helpful and efficient **grocery shopping list generator**. 
The user wants a comprehensive shopping list for the event or need: "${prompt}". 
Your task is to generate a list of generic, searchable items (including food and non-food supplies) that would be necessary for this event.
Respond **STRICTLY** with a JSON array of item strings, using only common, searchable keywords. Do not include any extra text or markdown wrappers.
Example 1: If the user says "movie night snacks", respond with ["chips", "salsa", "popcorn kernels", "soda", "candies", "disposable bowls"].
    `;
    ...
    // --- match supplies with products.json ---

    const allProducts = getProductData();

    let matches = [];

    for (const ing of supplies) {
    ...
    try {
      ...
      if (matches.length > 0) {
        ...
        const query2 = `
              ...
              The user is looking for supplies for: "${prompt}".
      ...
      console.warn(
        "⚠️ Pruning skipped (AI Overloaded or Error). Returning broad matches."
      );
